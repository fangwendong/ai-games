...
class LiveReportTest(unittest.TestCase):
    ...
    def test_formats_holding_snapshot_when_positions_exist(self) -> None:
        output = SAMPLE_OUTPUT.replace(
            '"current_positions":[]',
            '"current_positions":[{"symbol":"SOXL","quantity":21}]',
        )
        report = format_live_report(
            output,
            exit_code=0,
        ...
        self.assertIn("⚪ SOXL｜132.50｜今日未成交｜持仓 21股", report)
